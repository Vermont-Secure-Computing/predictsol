...
function normalizeCandles(candles = []) {
  return candles
    .map((c) => ({
      t: Number(c.t ?? c.unixTime ?? c.time ?? c.timestamp) * 1000,
      close: Number(c.c ?? c.close ?? c.price ?? c.value),
    }))
...
function mergeByTime(trueCandles, falseCandles) {
  ...
  for (const c of normalizeCandles(trueCandles)) {
    map.set(c.t, { t: c.t, trueClose: c.close });
  }

  for (const c of normalizeCandles(falseCandles)) {
    const prev = map.get(c.t) || { t: c.t };
    prev.falseClose = c.close;
    map.set(c.t, prev);
  }
...
export default function TruthOhlcvChart({
  ...
  const isSingle = candles.length > 0;
  ...
  const data = useMemo(() => {
    ...
    return mergeByTime(trueCandles, falseCandles);
  }, [isSingle, candles, trueCandles, falseCandles]);

  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900/60">
      <div className="mb-2 text-sm font-semibold">
        {isSingle ? `${label} Price` : "TRUE vs FALSE Price"}
      </div>
      ...
      <div className="mt-2 text-[11px] opacity-70">Source: {source}</div>
    </div>
  );
}